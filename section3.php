...
$PAGE->set_url(new moodle_url('/local/tad/section3.php', ['section' => $PAGE->url->get_param('section')]));
...
if ($mform->is_cancelled()) {
    redirect($CFG->wwwroot . '/local/tad/section3.php',  get_string("upload_cancelled", "local_tad"), \core\output\notification::NOTIFY_INFO);
} else if ($formdata = $mform->get_data()) {        
    create_tad_from_formdata($formdata);
}

echo $OUTPUT->header();
echo $OUTPUT->render_from_template('local_tad/tadsection3', []);
//$mform->display();
echo $OUTPUT->footer();